# Escriba su código aquí
#!/bin/bash
# Cambio de "/" por "-"
sed 's/\//-/g' estacion1.csv > estacion1a.csv
sed 's/\//-/g' estacion2.csv > estacion2a.csv
sed 's/\//-/g' estacion3.csv > estacion3a.csv
sed 's/\//-/g' estacion4.csv > estacion4a.csv
# Agregar nombre de la estación
sed 's/\([0-9]*\)\-\([0-9]*\)\-\([0-9]*\)/estacion1;\1-\2-\3/g' estacion1a.csv > estacion1b.csv
sed 's/\([0-9]*\)\-\([0-9]*\)\-\([0-9]*\)/estacion2;\1-\2-\3/g' estacion2a.csv > estacion2b.csv
sed 's/\([0-9]*\)\-\([0-9]*\)\-\([0-9]*\)/estacion3;\1-\2-\3/g' estacion3a.csv > estacion3b.csv
sed 's/\([0-9]*\)\-\([0-9]*\)\-\([0-9]*\)/estacion4;\1-\2-\3/g' estacion4a.csv > estacion4b.csv
# Consolidacion de archivos estaciones
touch estacionestotal.csv
awk 'FNR==1 && NR!=1{next;}{print}' estacion1b.csv estacion2b.csv estacion3b.csv estacion4b.csv > estacionestotal.csv  
# Cambio de "," por "."
sed 's/,/./g' estacionestotal.csv > estacionestotal1.csv
# Cambio de separadores ";" por ","
sed 's/;/,/g' estacionestotal1.csv > estacionestotal2.csv
# Organizar formato año AAAA
sed 's/-\([0-9]*\),/-20\1,/g' estacionestotal2.csv > estacionestotal3.csv
# Cambiar - por , (para separar por columnas)
sed 's/-/,/g' estacionestotal3.csv > estacionestotal4.csv
# Nombrar columnas
sed 's/FECHA,/ESTACION,DIA,MES,YEAR,/g' estacionestotal4.csv > estacionestotal5.csv
# Consultas SQL
csvsql --query 'select ESTACION, MES, avg(VEL) from estacionestotal8 GROUP BY ESTACION, MES' estacionestotal5.csv >> velocidad-por-mes.csv
csvsql --query 'select ESTACION, YEAR, avg(VEL) from estacionestotal8 GROUP BY ESTACION, YEAR' estacionestotal6.csv >> velocidad-por-ano.csv
csvsql --query 'select ESTACION, HHMMSS, avg(VEL) from estacionestotal8 GROUP BY ESTACION, HHMMSS' estacionestotal7.csv >> velocidad-por-hora.csv
head velocidad-por-mes.csv
head velocidad-por-ano.csv
head velocidad-por-hora.csv
#Borrar archivos
rm estacion*
rm estacionestotal*